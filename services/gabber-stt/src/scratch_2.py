...
def main(cfg: TranscriptionConfig) -> TranscriptionConfig:
    ...
    if manifest is not None:
        ...
        for record in records:
            record["audio_filepath"] = str(
                filepath_to_absolute(record["audio_filepath"], manifest_dir)
            )
    else:
    ...
    context_encoder_frames = ContextSize(
        ...
        right=int(
            cfg.right_context_secs * features_per_sec / encoder_subsampling_factor
        ),
    )
    ...
    logging.info(
        f"Corrected contexts (subsampled encoder frames): {context_encoder_frames}"
    )
    ...
    with torch.no_grad(), torch.inference_mode():
        ...
        for audio_data in tqdm(audio_dataloader):
            ...
            audio_batch = audio_data.audio_signals.to(
                device=torch.device(asr_model.device)
            )
            audio_batch_lengths = audio_data.audio_signal_lengths.to(
                device=torch.device(asr_model.device)
            )
            batch_size = audio_batch.shape[0]
            print("NEIL audio_batch", audio_batch.shape, audio_batch.dtype)
